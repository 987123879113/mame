...
uint16_t tokimeki_state::tokimeki_serial_r()
{
	...
	// 0x08 is for the GSR sensor (maybe more?), seems to possibly depending on 5 bit command sent to 1f680090
	uint16_t r = 0xffff;
	...
	if (m_tokimeki_serial_sensor_id != 0)
		r = (r & ~8) | (BIT(m_tokimeki_serial_sensor_data, 8) << 3);
...
void tokimeki_state::tokimeki_serial_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
	...
	*/
	...
	else if (!m_tokimeki_serial_clk && clk)
	{
		...
		{
			// Shifts data into the register for reading with tokimeki_serial_r
...
void tokimeki_state::tmosh(machine_config &config)
{
	...
	rs232_network.option_add("tokimekidev", KONAMI_TOKIMEKI_DEV);
	rs232_network.set_default_option("tokimekidev");
}
