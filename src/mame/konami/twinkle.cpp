...
private:
	...
	uint8_t *m_scsi_dma_data_ptr;
	uint32_t m_scsi_dma_address;
	int32_t m_scsi_dma_size;
	bool m_scsi_dma_is_write;
	bool m_scsi_dma_requested;
};
...
void twinkle_state::machine_start()
{
	...
	m_scsi_dma_timer = timer_alloc(FUNC(twinkle_state::scsi_dma_transfer), this);
	m_scsi_dma_timer->adjust(attotime::never);
	m_scsi_dma_data_ptr = nullptr;
...
{
	m_scsi_dma_data_ptr = (uint8_t*)p_n_psxram;
	m_scsi_dma_address = n_address;
	m_scsi_dma_size = n_size * 4;
	m_scsi_dma_is_write = false;
	m_scsi_dma_timer->adjust(attotime::from_usec(10));
}
...
void twinkle_state::scsi_dma_write( uint32_t *p_n_psxram, uint32_t n_address, int32_t n_size )
{
	...
	m_scsi_dma_address = n_address;
	m_scsi_dma_size = n_size * 4;
	m_scsi_dma_is_write = true;
	m_scsi_dma_timer->adjust(attotime::from_usec(10));
}

TIMER_CALLBACK_MEMBER(twinkle_state::scsi_dma_transfer)
{
	if (m_scsi_dma_requested && m_scsi_dma_data_ptr != nullptr && m_scsi_dma_size > 0)
	{
		if (m_scsi_dma_is_write)
			m_ncr53cf96->dma_w(m_scsi_dma_data_ptr[m_scsi_dma_address]);
		else
			m_scsi_dma_data_ptr[m_scsi_dma_address] = m_ncr53cf96->dma_r();

		m_scsi_dma_address++;
		m_scsi_dma_size--;
	}

	if (m_scsi_dma_requested && m_scsi_dma_size > 0)
		m_scsi_dma_timer->adjust(attotime::from_usec(10));
}

WRITE_LINE_MEMBER(twinkle_state::scsi_drq)
{
	m_scsi_dma_requested = state;
