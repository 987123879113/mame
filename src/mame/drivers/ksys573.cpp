...
private:
	...
	int jvs_output_idx_r, jvs_output_idx_w;
	uint8_t jvs_input_buffer[512];
	uint8_t jvs_output_buffer[512];

	bool jvs_is_valid_command();
...
bool ksys573_state::jvs_is_valid_command()
{
    if (jvs_input_idx_w < 3) {
            return false;
    }
...
void ksys573_state::jvs_input_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
    ...
    if (jvs_is_valid_command()) {
		...
		if (command_size % 2)
			command_size = (command_size + 1) & 0xfffffffe; // Must be padded to 16 bits, so remove any junk bytes

		memmove(jvs_input_buffer, jvs_input_buffer + command_size, 512 - command_size);
		jvs_input_idx_w -= command_size;
		jvs_input_idx_r -= command_size;

		if (jvs_input_idx_w < 0) jvs_input_idx_w = 0;
		if (jvs_input_idx_r < 0) jvs_input_idx_r = 0;
    }
}
...
uint16_t ksys573_state::jvs_output_r(offs_t offset, uint16_t mem_mask)
{
    ...
    uint16_t data = jvs_output_buffer[0] | (jvs_output_buffer[1] << 8);
    jvs_output_idx_w -= 2;
...
void ksys573_state::driver_start()
{
	...
	jvs_output_idx_r = jvs_output_idx_w = 0;
